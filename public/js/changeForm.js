...
const currentPasswordInput = mdc.textField.MDCTextField.attachTo(document.getElementById('current-password'));
...
if (emailInput) {
    emailInput.input_.addEventListener('keyup', () => {
        emailInput.value = emailInput.value.toLowerCase();
    });
}

if (currentPasswordInput) {
    currentPasswordInput.trailingIcon_.root.addEventListener('click', () => {
        const iconText = currentPasswordInput.trailingIcon_.root.textContent;
        if (iconText === 'visibility') {
            currentPasswordInput.input_.type = 'text';
            currentPasswordInput.trailingIcon_.root.textContent = 'visibility_off';
        }
        if (iconText === 'visibility_off') {
            currentPasswordInput.input_.type = 'password';
            currentPasswordInput.trailingIcon_.root.textContent = 'visibility';
        }
    });
}
...
if (submitButton) {
    submitButton.root.addEventListener('click', () => {
        ...
        if (currentPasswordInput.valid === false) {
            currentPasswordInput.valid = false;
